...
**This repo is a learning foundation — the next step is a component template repo.** Lesson repos are intentionally exploratory: they isolate concepts, repeat patterns deliberately, and optimize for understanding over reuse. A companion template repo would flip that priority — encoding the best patterns discovered here into ready-to-copy component structures (cards, navbars, forms, etc.) with correct `@layer` usage, token naming, and dark mode wiring already in place. Templates are more useful day-to-day than lessons because they skip the discovery phase and go straight to a vetted starting point.
